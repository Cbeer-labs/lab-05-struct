...
**Что такое структуры?**
* Структура — это некое объединение различных переменных (даже с разными типами данных), которому можно присвоить имя. Структура - это новый тип данных, эквивалентный для языка C++ структурам `std::string`, `std::vector`, и, в какой-то степени, элементарным типам: `int`, `size_t`, `float`, `double`.
* Например, можно объединить данные об объекте "Дом": город (в котором дом находится), улица, количество квартир, интернет (проведен или нет) и т.д. в одной переменной (типе).
...
* Когда количество аргументов функции (особенно высокоуровневой) начинает казаться громадным, логично предварительно объединить эти аргументы в единый тип и передавать его.
* Также структуры незаменимы, когда необходимо сгруппировать некоторые данные и работать с этим набором как с единым неделимым целым (название улицы не стоит отделять от города).
...
**Внимание!** Одна из самых простых ошибок в C++ — забыть точку с запятой в конце объявления структуры. Это приведет к синтаксической ошибке.
...
При наличии указателя на структуру (например `Student* ivan;`), доступ к полям структуры стоит осуществлять при помощи оператора `->`:
...
Ваше задание состоит в реализации ряда функций.

Реализуйте функции, которые выполяют следующие действия с этим списком:
...
* вернет массив уникальных названий групп студентов из списка students
  ...
  Порядок названий групп не важен, можно скорректировать тесты
```cpp
...
* сформирует список групп, т.е. создаст массив структур `Group`
  ...
  Порядок студентов в группе не важен, можно скорректировать тесты
```cpp
struct Group
{
    std::string Id;
